"""
CASL-Net Step 5: Severity Estimation
======================================
This script combines YOLOv8 + SAM output and applies:

  1. Infection Percentage Formula:
       infection_pct = infection_area / stem_area * 100

  2. Severity Index Formula:
       severity_index = w1*anthracnose_pct + w2*stemrot_pct + w3*canker_pct
       where w1=1.0, w2=1.5, w3=1.2
       (StemRot weighted higher as it spreads faster and is more destructive)

  3. Severity Label:
       0   - 5  → Healthy
       5   - 10 → Moderate
       10  - 15 → Slightly Infected
       15+ - 20 → Highly Infected

  4. Saves full results as a CSV report + annotated images
"""

import cv2
import torch
import numpy as np
import os
import csv
from pathlib import Path
from ultralytics import YOLO
from segment_anything import sam_model_registry, SamPredictor

# ─────────────────────────────────────────────
# CONFIGURATION
# ─────────────────────────────────────────────
MODEL_PATH   = r"C:\Data\MSRIT\8th SEM project\CASL-Net\models\yolov8\best.pt"
SAM_CKPT     = r"C:\Data\MSRIT\8th SEM project\CASL-Net\sam\sam_vit_h_4b8939.pth"
SAM_TYPE     = "vit_h"
TEST_IMG_DIR = r"C:\Data\MSRIT\8th SEM project\CASL-Net\dataset\images\val"
OUTPUT_DIR   = r"C:\Data\MSRIT\8th SEM project\CASL-Net\outputs\severity_results"
NUM_IMAGES   = 20       # test on more images this time
CONF_THRESH  = 0.35
IOU_THRESH   = 0.5
MIN_MASK_AREA= 300      # ignore masks smaller than 300px (noise filter)

CLASS_NAMES  = {0: "Anthracnose", 1: "StemRot", 2: "Canker"}

# Overlay colors per class (BGR)
CLASS_COLORS = {
    0: (0, 165, 255),    # Orange — Anthracnose
    1: (0, 0, 255),      # Red    — StemRot
    2: (255, 0, 0),      # Blue   — Canker
}

# ─────────────────────────────────────────────
# SEVERITY CONFIG
# ─────────────────────────────────────────────

# Disease weights — StemRot is most destructive so weighted higher
WEIGHTS = {
    "Anthracnose": 1.0,
    "StemRot"    : 1.5,
    "Canker"     : 1.2,
}

# Severity label thresholds (based on severity index)
def get_severity_label(severity_index):
    if severity_index < 5:
        return "Healthy"
    elif severity_index < 10:
        return "Moderate"
    elif severity_index < 15:
        return "Slightly Infected"
    else:
        return "Highly Infected"

# Label colors for display (BGR)
SEVERITY_COLORS = {
    "Healthy"           : (0, 255, 0),      # Green
    "Moderate"          : (0, 255, 255),     # Yellow
    "Slightly Infected" : (0, 165, 255),     # Orange
    "Highly Infected"   : (0, 0, 255),       # Red
}

# ─────────────────────────────────────────────
# HELPER: Estimate stem area
# ─────────────────────────────────────────────
def estimate_stem_area(image_bgr):
    hsv = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2HSV)

    lower_bg  = np.array([0,   0,   200])
    upper_bg  = np.array([180, 30,  255])
    bg_mask   = cv2.inRange(hsv, lower_bg, upper_bg)

    lower_sky = np.array([90,  10, 180])
    upper_sky = np.array([130, 80, 255])
    sky_mask  = cv2.inRange(hsv, lower_sky, upper_sky)

    combined_bg = cv2.bitwise_or(bg_mask, sky_mask)
    stem_mask   = cv2.bitwise_not(combined_bg)

    kernel    = np.ones((5, 5), np.uint8)
    stem_mask = cv2.morphologyEx(stem_mask, cv2.MORPH_CLOSE, kernel)
    stem_mask = cv2.morphologyEx(stem_mask, cv2.MORPH_OPEN,  kernel)

    stem_area  = int(np.sum(stem_mask > 0))
    total_area = image_bgr.shape[0] * image_bgr.shape[1]
    if stem_area < total_area * 0.1:
        stem_area = int(total_area * 0.60)

    return stem_area, stem_mask


# ─────────────────────────────────────────────
# HELPER: Overlay mask on image
# ─────────────────────────────────────────────
def overlay_mask(image, mask, color, alpha=0.45):
    overlay      = image.copy()
    colored_mask = np.zeros_like(image)
    colored_mask[mask > 0] = color
    cv2.addWeighted(colored_mask, alpha, overlay, 1 - alpha, 0, overlay)
    contours, _ = cv2.findContours(mask.astype(np.uint8),
                                    cv2.RETR_EXTERNAL,
                                    cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(overlay, contours, -1, color, 2)
    return overlay


# ─────────────────────────────────────────────
# HELPER: Draw severity panel on image
# ─────────────────────────────────────────────
def draw_severity_panel(image, result):
    """Draws a semi-transparent info panel on the bottom of the image."""
    panel_h = 130
    h, w    = image.shape[:2]
    panel   = image.copy()

    # Dark background panel
    cv2.rectangle(panel, (0, h - panel_h), (w, h), (20, 20, 20), -1)
    cv2.addWeighted(panel, 0.75, image, 0.25, 0, panel)

    label       = result['severity_label']
    sev_index   = result['severity_index']
    label_color = SEVERITY_COLORS.get(label, (255, 255, 255))

    y = h - panel_h + 22
    cv2.putText(panel, f"Severity: {label}  (Index: {sev_index:.2f})",
                (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, label_color, 2)

    y += 24
    cv2.putText(panel, f"Stem area: {result['stem_area_px']:,} px",
                (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1)

    y += 20
    for cls_name, pct in result['infection_pct'].items():
        color = CLASS_COLORS.get(
            [k for k, v in CLASS_NAMES.items() if v == cls_name][0],
            (255, 255, 255)
        )
        cv2.putText(panel, f"{cls_name}: {pct:.2f}%",
                    (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.42, color, 1)
        y += 18

    cv2.putText(panel, f"Total infection: {result['total_infection_pct']:.2f}%",
                (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)

    return panel


# ─────────────────────────────────────────────
# CORE: Process one image — detect + segment + severity
# ─────────────────────────────────────────────
def process_image(image_path, yolo_model, sam_predictor):
    image_bgr = cv2.imread(str(image_path))
    if image_bgr is None:
        return None

    image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
    h, w      = image_bgr.shape[:2]

    # ── YOLOv8 Detection ──
    yolo_results = yolo_model.predict(
        source  = str(image_path),
        conf    = CONF_THRESH,
        iou     = IOU_THRESH,
        device  = 'cpu',
        verbose = False,
    )
    boxes = yolo_results[0].boxes

    # ── Stem Area ──
    stem_area, _ = estimate_stem_area(image_bgr)

    # ── Default empty result ──
    class_areas = {0: 0, 1: 0, 2: 0}
    detections  = []
    vis_image   = image_bgr.copy()

    if boxes is not None and len(boxes) > 0:
        sam_predictor.set_image(image_rgb)

        for box in boxes:
            class_id   = int(box.cls[0])
            confidence = float(box.conf[0])
            x1, y1, x2, y2 = [int(v) for v in box.xyxy[0]]
            x1 = max(0, x1); y1 = max(0, y1)
            x2 = min(w, x2); y2 = min(h, y2)

            if x2 - x1 < 5 or y2 - y1 < 5:
                continue

            # SAM segmentation
            input_box = np.array([x1, y1, x2, y2])
            masks, scores, _ = sam_predictor.predict(
                point_coords     = None,
                point_labels     = None,
                box              = input_box[None, :],
                multimask_output = False,
            )

            best_mask = masks[0].astype(np.uint8)
            mask_area = int(np.sum(best_mask > 0))

            # ── FILTER: skip tiny/noise masks ──
            if mask_area < MIN_MASK_AREA:
                continue

            class_areas[class_id] += mask_area
            detections.append({
                'class_id'  : class_id,
                'class_name': CLASS_NAMES[class_id],
                'confidence': confidence,
                'bbox'      : (x1, y1, x2, y2),
                'mask_area' : mask_area,
            })

            # Visualize
            color     = CLASS_COLORS[class_id]
            vis_image = overlay_mask(vis_image, best_mask, color)
            cv2.rectangle(vis_image, (x1, y1), (x2, y2), color, 2)
            label = f"{CLASS_NAMES[class_id]} {confidence:.2f}"
            cv2.putText(vis_image, label, (x1, max(y1 - 8, 0)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

    # ── Infection Percentage per Disease ──
    infection_pct = {}
    for cid, area in class_areas.items():
        pct = round((area / stem_area * 100) if stem_area > 0 else 0.0, 4)
        infection_pct[CLASS_NAMES[cid]] = pct

    total_infection_pct = round(sum(infection_pct.values()), 4)

    # ── Severity Index ──
    # severity_index = w1*anthracnose_pct + w2*stemrot_pct + w3*canker_pct
    severity_index = round(
        WEIGHTS["Anthracnose"] * infection_pct["Anthracnose"] +
        WEIGHTS["StemRot"]     * infection_pct["StemRot"]     +
        WEIGHTS["Canker"]      * infection_pct["Canker"],
        4
    )

    severity_label = get_severity_label(severity_index)

    result = {
        'image_name'         : image_path.name,
        'stem_area_px'       : stem_area,
        'detections'         : detections,
        'class_areas_px'     : {CLASS_NAMES[k]: v for k, v in class_areas.items()},
        'infection_pct'      : infection_pct,
        'total_infection_pct': total_infection_pct,
        'severity_index'     : severity_index,
        'severity_label'     : severity_label,
        'vis_image'          : vis_image,
    }

    # Draw severity panel on visualization
    result['vis_image'] = draw_severity_panel(vis_image, result)

    return result


# ─────────────────────────────────────────────
# PRINT RESULTS
# ─────────────────────────────────────────────
def print_result(result):
    label_color_map = {
        "Healthy"           : "\033[92m",   # green
        "Moderate"          : "\033[93m",   # yellow
        "Slightly Infected" : "\033[33m",   # orange
        "Highly Infected"   : "\033[91m",   # red
    }
    reset = "\033[0m"
    color = label_color_map.get(result['severity_label'], "")

    print(f"\n{'='*57}")
    print(f"📸  {result['image_name']}")
    print(f"🌿  Stem area : {result['stem_area_px']:,} px")
    print(f"🔍  Detections: {len(result['detections'])} lesions detected")
    for d in result['detections']:
        print(f"      [{d['class_name']:12s}] conf={d['confidence']:.2f} | "
              f"mask={d['mask_area']:,} px")
    print(f"\n  Infection % per Disease:")
    for cls_name, pct in result['infection_pct'].items():
        print(f"      {cls_name:12s}: {pct:.4f}%")
    print(f"  Total Infection  : {result['total_infection_pct']:.4f}%")
    print(f"\n  Severity Index   : {result['severity_index']:.4f}")
    print(f"  Severity Label   : {color}{result['severity_label']}{reset}")
    print(f"{'='*57}")


# ─────────────────────────────────────────────
# SAVE CSV REPORT
# ─────────────────────────────────────────────
def save_csv_report(all_results, output_dir):
    csv_path = os.path.join(output_dir, "severity_report.csv")
    fieldnames = [
        'image_name',
        'stem_area_px',
        'num_detections',
        'anthracnose_area_px',
        'stemrot_area_px',
        'canker_area_px',
        'anthracnose_pct',
        'stemrot_pct',
        'canker_pct',
        'total_infection_pct',
        'severity_index',
        'severity_label',
    ]

    with open(csv_path, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for r in all_results:
            writer.writerow({
                'image_name'          : r['image_name'],
                'stem_area_px'        : r['stem_area_px'],
                'num_detections'      : len(r['detections']),
                'anthracnose_area_px' : r['class_areas_px']['Anthracnose'],
                'stemrot_area_px'     : r['class_areas_px']['StemRot'],
                'canker_area_px'      : r['class_areas_px']['Canker'],
                'anthracnose_pct'     : r['infection_pct']['Anthracnose'],
                'stemrot_pct'         : r['infection_pct']['StemRot'],
                'canker_pct'          : r['infection_pct']['Canker'],
                'total_infection_pct' : r['total_infection_pct'],
                'severity_index'      : r['severity_index'],
                'severity_label'      : r['severity_label'],
            })

    print(f"\n📊 CSV report saved: {csv_path}")
    return csv_path


# ─────────────────────────────────────────────
# MAIN
# ─────────────────────────────────────────────
def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # Load models
    print("\n📦 Loading YOLOv8...")
    yolo_model = YOLO(MODEL_PATH)
    print("✅ YOLOv8 loaded")

    print("\n📦 Loading SAM...")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"   Device: {device}")
    sam = sam_model_registry[SAM_TYPE](checkpoint=SAM_CKPT)
    sam.to(device=device)
    sam_predictor = SamPredictor(sam)
    print("✅ SAM loaded")

    # Get images
    val_images = list(Path(TEST_IMG_DIR).glob("*.jpg"))[:NUM_IMAGES]
    print(f"\n🔍 Processing {len(val_images)} images...\n")

    all_results     = []
    label_counts    = {"Healthy": 0, "Moderate": 0,
                       "Slightly Infected": 0, "Highly Infected": 0}

    for img_path in val_images:
        result = process_image(img_path, yolo_model, sam_predictor)
        if result is None:
            continue

        print_result(result)

        # Save annotated image
        out_path = os.path.join(OUTPUT_DIR, f"severity_{result['image_name']}")
        cv2.imwrite(out_path, result['vis_image'])

        label_counts[result['severity_label']] += 1
        all_results.append(result)

    # Save CSV report
    save_csv_report(all_results, OUTPUT_DIR)

    # Final summary
    print(f"\n{'='*57}")
    print(f"🎉  SEVERITY ESTIMATION COMPLETE")
    print(f"    Processed : {len(all_results)} images")
    print(f"\n    Severity Distribution:")
    for label, count in label_counts.items():
        print(f"      {label:20s}: {count} images")
    print(f"\n    Output dir : {OUTPUT_DIR}")
    print(f"{'='*57}")
    print("\n✅ Next step: EfficientNet Backbone Integration (Step 6)")


if __name__ == "__main__":
    main()